...
//   displayNum(data_ascii[arrage][t1], (t1+1));      // Parameters: (current column of current letter as "rowNum", current column+1 as "colNum")
void displayNum(byte rowNum,int colNum)               // This function draws one column
{
  int j;
  byte temp = rowNum; // == data_ascii[arrage][t1]

  for(j=2;j<6;j++)  // set row1-4 low
  {
    digitalWrite(j, LOW);
  }
  digitalWrite(row5, LOW); // Set other row pins low (?)
  digitalWrite(row6, LOW);
  digitalWrite(row7, LOW);
  digitalWrite(row8, LOW);
  
  for(j=6;j<14;j++)  // Set all col pins high
  {
    digitalWrite(j, HIGH); 
  }

  switch(colNum)      // Set LED Pin on colNum + 1 LOW (first requirement for activating LED)
    {
    ...
    for(j = 1 ;j < 9; j++)
    {
      temp = (0x80)&(temp); // 1000 0000 AND data_ascii[arrage][t1]
      if(temp==0)
      {
        temp = rowNum<<j;   // rowNum is bit-shifted to the left j-times
        continue;          // Skip for loop iteration
      }

      switch(j) // Activate LED by setting corresponding row pin to HIGH
      {
...
void loop(){
  int t1;     // Column Index: 0 <= t1 <= 7
  int l;      // Timer / Delay
  int arrage; // Current Letter

  for(arrage=0;arrage<10;arrage++) // For each letter
  {
    for(l=0;l<512;l++) // Timer Function: delays displaying the next letter by running 512 iterations before moving on
    {
      for(t1=0;t1<8;t1++) // For each column
      {
        displayNum(data_ascii[arrage][t1],(t1+1)); // Parameters: (current column of current letter as "rowNum", current column+1 as "colNum")
      }
    }
